...
function EventTag({ tag }: { tag: string }) {
  ...
  return <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${cls}`}>{tag}</span>;
}
...
function ConflictItem({ conflict }: { conflict: Conflict }) {
  ...
  const [txStatus, setTxStatus]   = useState(conflict.status);
  const [txSummary, setTxSummary] = useState(conflict.summary);
  const [txGlobal, setTxGlobal]   = useState(conflict.globalImpact ?? '');
  const [txEvents, setTxEvents]   = useState(conflict.events.map((e) => e.description));

  useEffect(() => {
    if (locale === 'en') {
      setTxStatus(conflict.status);
      setTxSummary(conflict.summary);
      setTxGlobal(conflict.globalImpact ?? '');
      setTxEvents(conflict.events.map((e) => e.description));
      return;
    }
    ...
    const texts = [
      ...
      ...conflict.events.map((e) => e.description),
    ];

    translateTexts(texts).then((results) => {
      if (cancelled) return;
      setTxStatus(results[0]);
      setTxSummary(results[1]);
      setTxGlobal(results[2]);
      setTxEvents(results.slice(3));
    });
    ...
    <div className={clsx(
      ...
      {isPinned && txGlobal && (
        <div className="px-4 py-2 bg-orange-500/10 border-b border-orange-500/20 flex items-start gap-2">
          ...
          <p className="text-xs text-orange-300/80">{txGlobal}</p>
        </div>
      )}

      <button
        onClick={() => setOpen((v) => !v)}
        className={clsx('w-full text-left p-4 flex items-center justify-between gap-3 transition-colors', isPinned ? 'hover:bg-red-500/5' : 'hover:bg-white/5')}
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 truncate mt-0.5">{txStatus}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <SeverityBadge severity={conflict.severity} size="sm" />
          <svg className={clsx('w-4 h-4 text-gray-500 transition-transform', open && 'rotate-180')} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {open && (
        <div className="border-t border-white/10 p-4 bg-white/[0.02]">
          <p className="text-gray-400 text-xs leading-relaxed mb-4">{txSummary}</p>

          <div className="flex flex-wrap gap-1 mb-4">
            {conflict.tags.map((tag) => <EventTag key={tag} tag={tag} />)}
          </div>
                ...
                <div key={i} className="flex gap-3 text-xs">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap gap-1 mb-1">
                      {event.tags.map((tg) => <EventTag key={tg} tag={tg} />)}
                    </div>
                    <p className="text-gray-400 leading-relaxed">{txEvents[i] ?? event.description}</p>
                    {event.source && <p className="text-gray-600 mt-0.5">— {event.source}</p>}
                  </div>
                </div>
              ))}
            </div>
          ...
          <p className="text-[10px] text-gray-600 mt-3">
            {t('panel.lastUpdated')} {formatDate(conflict.startDate)} · {formatDate(conflict.lastUpdated)}
          </p>
        </div>
      )}
    </div>
...
export default function ConflictAccordion({ conflicts, filterTag, filterSeverity }: Props) {
  ...
  if (filtered.length === 0) {
    return <p className="text-gray-500 text-sm text-center py-8">{t('wars.noResults')}</p>;
  }
